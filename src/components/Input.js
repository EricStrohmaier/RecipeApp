...
export default function Input({ setCategories }) {
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
      <div className=" lg:flex lg:flex-row lg:w-full lg:m-3 sm:w-1/2 sm:flex sm:flex-col m-8 sm:m-5 md:m-12 ">
        ...
        <div className="flex items-center px-1 w-1/2">
          <input
            className="form-check-input"
            type="checkbox"
            value="random"
            ...
            onChange={handleCheckboxChange}
          />
          <label className="px-2   font-bold" htmlFor="random">
          Random recipe
          </label>
        </div>
        <button
        type="submit"
        className="px-2 py-2 relative lg:top-1 w-1/4 lg:w-1/6  top-3 sm:w-1/4  bg-blue-200 text-black rounded-lg"
      >
        Submit
      </button>
      </div>
